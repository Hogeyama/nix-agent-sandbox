...
| `configuration/acp` | ACP クライアントとの接続 | ACP 対応クライアントから Claude を使いたい → ホストの準備、専用プロファイル、クライアント設定を終えて隔離の境界を判断 | ホスト導入・ログイン・設定信頼 → イメージ再構築 → Claude 設定を継承する ACP プロファイル → command / args / cwd と pipe・起動待ち中の入力上限 → 非対応設定 → 診断 → クライアント・MCP の境界 |
