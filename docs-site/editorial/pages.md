...
| `work/approvals` | 通信・ホスト実行の承認 | 要求が保留 → 適切な範囲で許可・拒否 | 要求の所在 → 通信の対象・範囲・操作 → ホスト実行の対象・範囲・操作 → UI を使わない場合の一覧・応答・購読 → 結果と次の判断 |
...
| `configuration/acp` | ACP クライアントとの接続 | ACP 対応クライアントから Claude を使いたい → ホストの準備、専用プロファイル、クライアント設定を終えて隔離の境界を判断 | 起動の仕組みと到達点 → ホストの Claude ログイン・設定信頼（非対話で止まる条件） → ACP adapter の用意（Nix / 非 Nix の両方を実行可能に） → Claude 設定を継承する ACP プロファイル（継承元にあると起動しない設定の解除を同じ節に置く） → 実在クライアントの登録例と command / args / cwd、pipe → 動作の確認（最初の応答、起動待ち中の入力上限、UI での確認、終了） → 承認がプロトコルに乗らないことと応答先・待ち時間の変更先 → 診断 → 指定した adapter についてのクライアント・MCP の境界 |
...
| `configuration/development` | 開発ツールと Docker | テストに必要なツールがない → プロジェクトの環境を読み込み、必要なら Docker を利用 | プロジェクト環境の選択 → direnv の既定と無効化 → ホストで実際の RC を承認 → ツールの確認 → 必要な場合の Docker 設定と取得許可 → 寿命とキャッシュ → イメージ再構築 |
...
| features/nix | configuration/development に統合 | direnv の既定の有効さと無効化、ホスト承認、Nix store・daemon の共有 |
