...
注入するのは、プロキシから接続先へ TLS（`https://`）で送る要求だけです。平文の HTTP で送る要求は、許可されてもヘッダーを付けずに送ります。`http://` の URL への要求も、443 番への CONNECT の中で平文の HTTP を送る要求も、これに当たります。注入したヘッダーが経路上で平文のまま読まれないようにするためです。
