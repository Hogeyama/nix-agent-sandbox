...
ホストの `~/.claude`・`~/.codex`・`~/.copilot` は、認証情報とセッション履歴を持ち込むため読み書き可能で共有します。そのうち設定ファイルだけは、既定で読み取り専用に上書きします。
...
コンテナ内から設定ファイルを編集する必要がある場合は、対象プロファイルで無効にします。
...
無効にすると上の経路が開きます。ホスト側の設定を変えたいだけなら、コンテナ内ではなくホストで編集してください。
