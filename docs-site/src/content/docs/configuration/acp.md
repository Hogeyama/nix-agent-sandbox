...
ACP モードでは、コンテナ内の PATH にある `claude-agent-acp` コマンドを起動します。adapter と、その実行に必要な Node.js を、ワークスペースの Nix devShell（direnv 連携）やマウントでコンテナ内から実行できるようにしておきます。

Claude バイナリの場所と nas proxy の CA 証明書は nas が adapter に渡すため、追加の設定は不要です。コマンドが見つからない場合は起動直後に終了し、クライアントが受け取る stderr に `command not found` が出ます。
...
一方、ACP クライアントが context として送る内容は、プロトコル経由でコンテナへ入るため nas の maskfs を通りません。ACP にはクライアント側のファイル読み書き capability もあります。adapter が通常のファイル操作に Claude の標準ツールを使う場合、その読み書きはコンテナ内で行われます。クライアント自身の機能や、用意した adapter がクライアント側のファイル API を使う場合、その読み書きは nas の外側です。
