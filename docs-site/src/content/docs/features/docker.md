...
DinD daemon による image pull も session proxy を経由します。対象 registry を `network.scopes` の `targets` で選択し、pull request を rule で許可するか、review または fallback の経路で承認してください。request が最終的に拒否されると、proxy は `403 Forbidden` を返し、pull は失敗します。
