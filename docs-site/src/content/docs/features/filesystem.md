...
上の `extraMounts { ... }` は、共通設定から引き継いだ共有フォルダーを残して追加する書き方です。同じファイルに `extraMounts` がすでにあれば、その中に `new { ... }` の行を追加します。

| 書き方 | 結果 |
| --- | --- |
| `extraMounts { new { ... } }` | 引き継いだ共有フォルダーを残して追加。 |
| `extraMounts = new Listing { new ExtraMountConfig { ... } }` | 引き継いだ一覧を、新しく書いた一覧で置き換え。 |
