...
| <span id="dind">Docker in Docker</span> | `docker.enable = false` | `docker.enable = true` | session 専用の rootless DinD sidecar / mutable state と、session 間で永続する public Docker Hub pull cache。 | agent container は host Docker socket を得ないが、sidecar は `--privileged` で起動する。cache hit は upstream request と新しい network approval を発生させない。`docker.shared = true` は deprecated で、DinD 有効時は validation error。 [Docker in Docker](/nix-agent-sandbox/features/docker/) |
