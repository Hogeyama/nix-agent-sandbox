...
Deno.test("notifyHostExecPendingRequest: desktop notification opens UI via xdg-open", async () => {
  await withFakeCommands(async ({ dir, healthServer }) => {
    ...
    assertEquals(xdgArgs.includes("type=hostexec"), true);
    assertEquals(xdgArgs.includes("sessionId=sess_test"), true);
    assertEquals(xdgArgs.includes("requestId=req_test"), true);
  });
});

Deno.test("notifyHostExecPendingRequest: dismiss does not open browser", async () => {
  await withFakeCommands(async ({ dir, healthServer }) => {
    const xdgLog = `${dir}/xdg-open.log`;
    Deno.env.set("NAS_NOTIFY_EXIT", "0");
...
Deno.test("notifyHostExecPendingRequest: uiEnabled=false shows approve/deny actions", async () => {
...
async function withFakeCommands(
  ...
  const dir = await Deno.makeTempDir({
    prefix: "nas-hostexec-notify-test-",
  });
