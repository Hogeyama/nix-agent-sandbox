...
設定・所有記録へ書くパスや表示するパスに秘密一覧の値またはその展開パターンが含まれる場合は、何も書かず終了コード1で中止します。 走査で秘密が見つかったファイルが、今回変更する設定・所有記録の実体と同じ場合も、シンボリックリンクやハードリンク経由を含め、何も書かず終了コード1で中止します。 設定と所有記録が同じファイルを指す場合や、作成する出力がリンク切れのシンボリックリンクの場合も、保存を拒否します（両方とも変更不要なら書きません）。
