...
- **Workspace**: `.git/commondir`, and the `commondir` of each linked worktree, are mounted read-only too. Git reads this file in any repository, not only in linked worktrees, and takes the config and hooks from the directory it names, so an agent could otherwise bypass the read-only config and hooks by writing one. When the file is missing, nas creates it on the host containing `.`, which git treats the same as no file, and leaves it in place after the session.
