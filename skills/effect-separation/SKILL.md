...
description: Stage Architecture — Effect Separation Design Rules. Use when adding new stages, modifying existing stages, creating helper functions, or refactoring pipeline code. Always consult when touching stages/ or pipeline/ directories, or when design questions arise about where to place side-effects.
---
...
| Probe resolver (`host_env.ts`, `mount_probes.ts`) | FS probe, env read, command execution |
...
4. **Naming helpers** — Pure: `build*/format*/expand*`. I/O: `resolve*/ensure*/check*`.
