...
登録情報は workspace・profile 名・起動コマンド・生成パスのみを保存し、
設定内容の指紋は取らない。設定は `up` のたびに読み直すため、ホストで変更した
設定は `down` 後の `up` でそのまま反映される (起動中のセッションには影響しない)。
`init` は既存の `.devcontainer` を上書きしない。独自編集の黙示的な承認はしない。
