...
| 値の解決タイミング | `resolveMode` で制御 | `"startup"`（起動時1回）or `"per-request"`（毎リクエスト）。Schema.pkl で定義 |
...
export interface CredentialRule {
  ...
  resolveMode: "startup" | "per-request"; // デフォルト: "startup"
}
...
export interface ResolvedCredential {
  ...
  method?: string;
  header: string;
  value: string;            // valCmd 実行済みの値（startup の場合）
  resolveMode: "startup" | "per-request";
  valCmd?: string;          // per-request の場合、毎回実行するコマンドを保持
}
```
...
nas 起動
  ...
  │     → resolveMode: "startup" の credential は valCmd を実行して値を解決
  │     → resolveMode: "per-request" の credential は valCmd を保持したまま返す
  │     → ResolvedCredential[] を得る
  │
  ...
  │     → resolveMode: "startup" → 保持済みの値を使用
  │     → resolveMode: "per-request" → valCmd をその場で実行して値を取得
...
- `resolveMode: "per-request"` の credential がマッチした場合 → broker が `valCmd` をその場で実行して値を取得
...
network:
  credentials:
    - host: "github.com"
      ...
      # resolveMode: "startup"  ← デフォルト。起動時に1回だけ実行
    - host: "api.github.com"
      header: "Authorization"
      value:
        valCmd: "echo token $(gh auth token)"
    - host: "registry.npmjs.org"
      header: "Authorization"
      resolveMode: "per-request"  # 毎リクエスト実行（短命トークン向け）
      value:
