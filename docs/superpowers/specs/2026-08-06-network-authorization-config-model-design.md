...
`EncodedField` が今なお埋めていた差は 2 つだけであり、いずれもマスク層で閉じるべき
ものである。認可の config に持ち込む理由がない。

- 7 バイト未満の秘密。誤マスクを避けるための閾値で捨てられている。
- 改行を含む base64。折り返された blob は確定部分文字列が分断されて一致しない。
...
- `EncodedField` と `maxDecodedBytes` を削除する。マスク層の base64 確定部分文字列に
  一本化する（「書き換えを持たない」）
- 同じ段階でマスク層の 2 つの穴を塞ぐ。`EncodedField` を先に外すと、その間だけ被覆が
  狭まる
  - `B64_MIN_PATTERN_LEN` を下げ、7 バイト未満の秘密にも base64 パターンを与える。
    6 文字の base64 は 36 ビットあり、100KB のボディでの誤一致は 10^-6 程度に収まる
  - 照合の前に base64 候補領域の改行と空白を畳み、折り返された blob に一致させる
