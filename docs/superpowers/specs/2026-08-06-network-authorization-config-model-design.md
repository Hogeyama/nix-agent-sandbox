...
打ち切ると承認 UI の違反レコードが 1 件目だけになり、承認者が 1 件承認するたびに次の 1 件が
...
**違反レコードには違反した要素をすべて載せる。** 帰結が `review` のとき、`allow`（記録して通過）
に留まる違反も承認者に見せる。承認者はリクエスト全体を見て押すので、記録に回った違反を
隠す理由がない。ただし違反レコードの保持件数そのものには上限がある（「違反レコード」）。要素を
選り分けないという意味であって、無制限に積むという意味ではない。

### 検査が完了しないとき

セレクタの走査が `maxSelectorExpansions` を使い切ると、残りの部分木を検査しないまま
評価が終わる。解析の失敗と違い、これは受理条件の側で起きる。`maxNodes` はここに現れ
ない。検査が始まる前に一度だけ検査する天井なので、超えたボディは中身を一切検査せずに
拒否され、一部だけ検査した状態にはならない（「予算」）。

検査未完了は違反として扱う。帰結は、そのルールの `expect` に現れる `onViolation` のうち
最も厳しいものとする。どの要素が違反したはずかを知らないので、宣言されている中で最悪の
帰結を仮定する。違反レコードには、予算を使い切ったセレクタと、走査が到達した最後の JSON Pointer
を含める。
...
`onViolation = "review"` は、違反レコードを承認 UI に渡せる場合にだけ選べる。違反レコードは
違反ごとに次を含む。
...
見ないまま押すことになる。違反レコードはセレクタの走査中に確定しているので、走査に JSON
Pointer を持たせれば取り出せる。

**違反レコードはどのフィールドにも生の秘密を載せない。** 抜粋だけでなく、JSON Pointer と
見つかった値もマスクする。違反レコードは承認 UI と監査ログへ出ていく記録なので、ボディに
由来するフィールドは例外なくマスクを通す。

- **JSON Pointer。** ポインタの参照トークンはボディのオブジェクトキーそのものなので、
  キーが秘密を含めばポインタが秘密を運ぶ。マスクしてもポインタとして壊れないことを
  要求する。`/` の区切りと `~0`/`~1` の escape は構造であって内容ではないので、書かれた
  ままの文字列を平坦にマスクすることはできない。escape を解いた本文の上でマスクし、
  区切りの位置を持ち回って再 escape する。秘密が区切りを跨いでいた場合は、跨がれた
  トークンが 1 つのマスク済みトークンに融合する。
- **見つかった値。** 未知タグはボディ由来の文字列なので、そのまま違反レコードに載せない。承認の
  単位に入る値でもある（「承認の単位」）ので、承認者が見るのはマスク済みの値である。

**違反レコードの保持件数には上限を置く。** 1 回のボディ検査が個別に記述する違反レコードは最大
`MAX_RETAINED_FINDINGS` 件（現在 64）とする。上限を超えた違反も走査して数えるが、
受理条件の識別子・JSON Pointer・値・抜粋を持つ記録は作らない。代わりに、打ち切りが
起きたことを示す違反レコードを 1 件足し、そこに漏れた違反の合計件数を持たせる。違反レコードの件数の
総和は常に違反の総数と一致するので、読む側は打ち切られた一覧をそうでない一覧と
区別できる。

上限が要るのは、違反を束ねる鍵が「見つかった値」だからである。値はボディ由来なので、
ノードごとに違う値を書いたボディは違反レコードをノード数だけ生み、その 1 件ずつが抜粋を作る。
値で束ねるだけでは件数も仕事量もメモリも縛れない。

**上限は 1 リクエストのボディ検査全体で共有する。** 受理条件ごとではない。先に走査した
受理条件が上限を埋めきると、後続の受理条件が見つけた違反は打ち切りの記録に落ちる。

この帰結は書き留めておく。**打ち切られた違反は承認に変換できない。** 承認の単位は
(ルール ID, 受理条件の識別子, 違反した値) だが（「承認の単位」）、打ち切られた違反は
その 3 成分のうち 2 つを失っているので、承認者が押せる対象にならない。`onViolation` が
`deny` と `allow` しかない間はこれで困らない。帰結は違反の有無で決まり、違反レコードの件数には
依らないからである。`review` を導入した時点で穴になる: 上限を超えた違反は承認できず、
そのリクエストは出し直しても通らないまま残る。`review` の実装時にこの節へ戻ること。
...
そのまま載せる（違反レコードの `at` も同じ Pointer を切らずに運んでいる）。その長さは設定
エラーで 256 文字までに抑える（「設定エラー」）。長さの上限は
スカラーの部分にだけ、これまでどおり（切ったときは切る前の全体のダイジェストを付けて）
かける。値全体を切ると、長い Pointer がスカラーを値の外へ押し出す。スカラーの違反レコードには
...
検査しなかった違反レコードには、違反した値としてリクエストごとの乱数の識別子（UUID）を載せる。1 つのリクエストの中では
事実 1 つにつき UUID を 1 つ引く（同じ種別の operation や、別名で並べた同じ root
field が複数あっても違反レコードは 1 件）。
...
クエリ文字列の違反レコードは、実行される document も変数もリクエストごとに URL の中にあるので
同じである。固定の値を置くと、
...
たびに確認が出て、`violation` の粒度はこれらの違反レコードに限って `once` と同じに振る舞う。
そのため、答えを要する違反レコードがすべてこの種類であるリクエストには `violation` の粒度を
提示しない。選んでも `once` と区別がつかず、承認者に記憶されると誤解させるからである。
他の違反レコードと混在するリクエストでは提示し、記憶されるのは他の違反レコードの側だけになる。
何が起きたかの読める説明（`operation:<種別>`、`rootField:<name>`、
`document:(unanalysable)`、`document:(query-string)`、`argument:<name>=(unresolved)`）は違反レコードの表示名 `label` に
...
意味のまま null にしておく。クエリ文字列の違反レコードは種別を `body-unavailable` とし、同じ
`BodyExpect` のほかの GraphQL の違反レコードを伴わない（ボディの document が実行されるものか
分からないので解析しない）。クエリ文字列そのものは値にも表示名にも載せない。

`violation` の粒度で覚えられる GraphQL の違反レコードは、集合に無い引数値の
...
`maxNodes` は解析済みツリーの大きさを制限する天井であり、検査が始まる前に一度だけ
検査する。ボディがこの天井を超えていれば、中身を一切検査せずに拒否する。

`maxSelectorExpansions` は走査そのものを制限する天井であり、こちらは
**1 リクエストのボディ検査が行うすべての走査で共有する 1 つの許容量**である。走査
ごとでもセレクタごとでもない。ルールが複数の受理条件を持ち、各受理条件が `exclude`
を持てば走査は何度も起きるが、それらは同じ残量を食い合う。

### 既定値どうしの関係（実測）

既定値は互いに独立ではない。**受理条件を足すと、正常なリクエストが予算切れで拒否
されうる。** 足す前にこの節の数字を確かめること。

計測（CPython 3.13、i9-12900）。歩数は `maxNodes` ぎりぎり（200,000 ノード）の
ボディを走査させて数えた。ボディの形で係数が変わるので、実会話に近い形と、
1 ノードあたりの歩数が最大になる形の両方を測っている。最大になるのは、メッセージ
1 つが 3 ノード（オブジェクト・`role`・空の `content` 配列）しかない形である。
`/messages/*/content/*` 系のセレクタは content 配列まで必ず降りるので、中身が
空でもメッセージあたり 2 歩を課金する。

| 量 | 実測値 |
| --- | --- |
| 走査 1 歩あたりの費用 | 1.3 µs（100 万歩の規模）〜1.7 µs（400 万歩の規模） |
| `TaggedUnionGuard` 5 本が 1 ノードあたり使う歩数 | 1.57〜2.67 |
| 上に `encodedFields` の `/**` を足したときの歩数 | 3.57〜4.67 |
| 実トラフィックの最大ノード数 | 8,204（897 メッセージ・1.0MB の会話） |

`maxNodes` ぎりぎりのボディが最悪の形で使う歩数は、それぞれ次になる。

| 走査する受理条件 | 歩数 | `maxSelectorExpansions` に対する割合 |
| --- | --- | --- |
| `TaggedUnionGuard` 5 本 | 533,323 | 53% |
| 5 本 + `encodedFields` の `/**` | 933,323 | 93% |

**`/**` は受理条件ではない。** これは `EncodedField` のセレクタであり、本設計は
`EncodedField` を廃止する（「書き換えを持たない」）。したがって 93% は**廃止前の
現行コード**の数字、53% は**本設計が記述する形**の数字である。

`/**` は 1 ノードにつき 2 歩（そのノードでの一致判定と、子への降下）を課金するので、
ボディの形に関係なくノード数の 2 倍を必ず消費する。廃止で消えるのはこの固定費である。

余地の差は大きい。`/messages/*/content/*` と同じ系統の受理条件 1 本は最悪の形で
1 ノードあたり 0.67 歩、`maxNodes` 換算で 133,333 歩を使う。

- **廃止前（現行コード、`/**` 込みで 933,323 歩）。** 受理条件を 1 本足すと
  1,066,656 歩となり天井を超え、正常なリクエストが `resource-limit` で 403 になる。
  余地はない。
- **廃止後（5 本で 533,323 歩）。** 8 本まで足して 933,313 歩（93%）で収まり、
  9 本目の 1,066,643 歩で超える。**あと 3 本の余地がある。**

`maxSelectorExpansions` を上げるのは解ではない。歩数を増やして許すということなので、
使い切ったときの検査時間がそのまま伸びる（1,000,000 で 1,350 ms、3,520,000 で
5,335 ms）。しかも線形より悪い。1 歩の費用が規模とともに 1.3 µs から 1.7 µs へ
上がるのは、走査が状態のメモ化に持つ集合が歩数とともに大きくなるためで、天井を
上げるほど 1 歩が高くつく。mitmproxy の request フックは同期実行なので、この時間は後続フローの
待ち時間になる。効いている天井は `maxNodes` のほうである。

実トラフィックの最大が 8,204 ノードであることに対し `maxNodes = 200,000` は 24 倍で、
下げる余地は大きい。ただし既定値は同時に固定の天井なので、下げるとユーザーが上げ
直せない。**現時点では両方とも据え置く**という判断であり、受理条件を増やす変更が
来たときにこの節に戻ること。

走査ごとに割り当てると、実効的な天井が走査の数だけ倍になる。設定に書いた数が実際に
効く数でなくなるので、天井として意味を成さない。共有にすると、受理条件を 1 つ
足しただけで既存の受理条件が予算切れになりうるが、予算切れは違反として扱われる
（「検査が完了しないとき」）ので、静かに検査を飛ばす方向には倒れない。
...
- `BodyExpect` の `equals` / `oneOf` の JSON Pointer が 256 文字（UTF-16 の単位）を
  超える。違反レコードはこの Pointer を切らずに値に含め（「承認の単位」）、broker は
  値の長さを確かめてから違反レコードを受け取るので、長い Pointer の違反レコードは受け取られず、その
  リクエストは承認できなくなるため。`match.body` の Pointer は違反レコードに載らないので対象に
  しない。
...
- 違反レコード（違反した受理条件、JSON Pointer、見つかった値、そのノードの抜粋、件数）を
...
現行の `bodyPreview`（先頭 1024 バイト）は違反レコードに置き換える。段階 1 で `deny` にしていた
