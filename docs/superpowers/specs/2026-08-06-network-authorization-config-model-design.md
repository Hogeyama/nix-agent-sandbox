...
隠す理由がない。ただし所見の保持件数そのものには上限がある（「違反の所見」）。要素を
...
**所見はどのフィールドにも生の秘密を載せない。** 抜粋だけでなく、JSON Pointer と
見つかった値もマスクする。所見は承認 UI と監査ログへ出ていく記録なので、ボディに
...
`deny` と `allow` しかない間はこれで困らない。帰結は違反の有無で決まり、所見の件数には
