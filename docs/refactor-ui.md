...
| 4 | (optional) EditableSessionName 共通化 | +200/-300 | #2a, #3 | defer (priority low) |
| 5 | (optional) module organization cleanup | +100/-200 | #1〜#4 | defer (priority low) |
...
### Phase 4 #4 / #5 の defer (priority low)

`#4 EditableSessionName 共通化` と `#5 module organization cleanup` は
optional 項目として **defer**。

#### 判断根拠

**#4**: `EditableSessionName` (`ContainersTab` 配下) と `TerminalTabLabel`
(`TerminalModal` 配下) の rename UI 共通化。両 component の **共通部分**
は rename state machine 30-40 行 (state ×3 + handleSave + Enter/Escape
keyboard handler) だが、**差異** が構造的:

- props 名 (`sessionName`/`onRenameSession`/`isActive`/`onDone` vs
  `currentName`/`fallbackName`/`onRename`)
- `onRename` 必須性 (必須 vs optional read-only 分岐)
- focus 戦略 (`useEffect` 明示 focus + `parent button's
  onMouseDown={preventFocusSteal} calls preventDefault()` workaround vs
  `autoFocus` 属性)
- event propagation (wrapper 全体に 3 event stopPropagation vs 個別
  要素 click のみ)
- `onDone` side-effect (save/escape/cancel 全部で呼ぶ vs なし)
- styling (CSS class vs inline style)
- read-only 分岐 (なし vs あり)
- fallback chain (`sessionName || sessionId` vs `currentName ||
  fallbackName || "-"`)

共通化方針として最有力なのは **custom hook** (state + handleSave +
keyboard handler を抽出、UI 層は 2 component で別々保持) だが、削減
実効 -30〜-40 行程度に対して以下のコストが境界線:

- bit-identical 検証コスト (frontend のコンポーネント描画 test は方針
  外で目視 + 手動 QA 依存、focus 戦略 / stopPropagation scope / `onDone`
  呼び場所を hook 境界で保存するレビュー負荷)
- Preact hook test ランナー未整備 (`containersHelpers_test.ts` のような
  純関数テストは存在するが hook test 基盤は追加インフラコスト)
- hook 境界に複雑性が移動するだけで本質的削減にならないリスク

**#5**: `App.tsx` の `DeepLink` 型移設等の module organization。現状
スコープが軽量で、`#4` を defer するなら #5 単独実施の動機も薄い。
`hooks/` 配下整理する流れになれば #4 と合わせて再着手する余地あり。

#### 再着手トリガ条件

**#4** (以下のいずれかが満たされた時に再評価):
- rename semantics が変わる変更 (server validation 追加、duplicate
  name 警告等) が入り、両 component で同じ修正が必要になった時
- `hooks/` 配下を整理する流れになり、rename hook 抽出が #5 と合わせて
  低コストになった時
- 3 つ目の inline rename UI を導入することになり、2 重化が 3 重化する
  前に共通化する価値が出た時

**#5**:
- `App.tsx` の `DeepLink` 型循環依存解消や、`onShell` dead prop
  (Phase 4 Commit 3 で温存) のような pre-existing module hygiene 課題
  をまとめて整理する流れになった時

### Phase 4 ステータス

Phase 4 主要 commit (#1 reducer / #2a TerminalModal 低リスク抽出 / #3
ContainersTab 分割) 完了。`#2b` (TerminalModal search bar 分割) /
`#2c` (TerminalModal toolbar 分割) / `#4` (EditableSessionName 共通化)
/ `#5` (module organization cleanup) はすべて defer (priority low)。

主要効果:
- `App.tsx` 最大複雑性ホットスポット解消 (4 useState の絡み合いを
  `terminalUiReducer` 純関数 + 16 reducer test に集約)
- `TerminalModal.tsx` 976 → 841 行 (xterm + WebSocket lifecycle 非
  侵入)
- `ContainersTab.tsx` 645 → 247 行 (純関数 / styles / 子 component
  3 個に分離)
- frontend unit test +43 本 (reducer 16 + terminalSize 7 + helpers 20)

`src/ui` 全体 refactor 主要部完了 (Phase 2 全 / Phase 3 主要 + #9 /
Phase 4 主要)。残 optional 項目は将来 commit で熟慮判断。
