...
- 右クリック/長押しの専用メニューは持たない
- 行 hover の個別アクションは出さない
- 行タイトルのダブルクリックでリネームのインライン編集を開始する
...
```
[● session名 · id · AGENT/SHELL]  [Ack turn] [Open shell / Return to agent] [Search]    [14px] [Kill clients]
```

- 左端：選択中セッションのコンテキスト（dot + 名前 + id + AGENT/SHELL）
- 中央：アクションボタン（`Ack turn` はアンバー強調）
- 右端：font-size + `Kill clients`

#### 用語

- **Shell** = この container で shell を起こす（中央 pane トグル）
...
- `Open shell` / `Return to agent` ボタンで中央 pane の表示を切替。シェル未作成なら初回クリックで spawn + 切替
- ボタンラベルは表示中の反対を指す：
  - agent 表示中 → `Open shell`
  - shell 表示中 → `Return to agent`
- toolbar の context ラベルに現在の view を `AGENT` / `SHELL` バッジで明示
- 両方の WS は keep-alive。非表示側もバッファ継続
- shell プロセス exit → 自動で agent に戻り、ボタンは `Open shell` に戻る
- **1 container に shell は 1 つだけ**（設計上の制約）
- session 切替時は各 session 毎の「最後に見ていた view」を復元
- rename はインライン編集で、行タイトルまたは Toolbar context label のダブルクリックで開始する
...
| P3 | セッション切替（左 pane クリック）/ Rename / Shell 起動 / pane drag-resize | 痛点 (a)(b)(c) 解決 |
