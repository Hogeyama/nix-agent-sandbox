...
- **dtach** （`session.enable: true` を使う場合のみ）
...
`session.enable: true` のときに使えるセッション管理コマンドです。
...
| `session.enable` | bool | `false` | dtach セッション内で nas を起動する。複数ターミナルからの同時 attach や detach 後の再接続が可能 |
