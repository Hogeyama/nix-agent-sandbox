...
export function makeDevcontainerLifecycle(
  ...
  /** Registered workspace whose managed config still exists and whose current
   * profile still passes the devcontainer gate. */
  const verifyLive = async (
    ...
    if ((await readTextFile(registration.configPath)) === null)
      throw new DevcontainerError(
  ...
  const init = async (
    ...
    return await withDevcontainerOperationLock(host, workspace, async () => {
      ...
      try {
        ...
        // A config left behind with no matching registration would block
        // re-init as "existing .devcontainer is not managed by nas", so
        // restore the pair the workspace had before this init.
  ...
  const up = async (requested: string): Promise<DevcontainerStatus> => {
    ...
    const registration = await withDevcontainerOperationLock(
      ...
      async () => {
        ...
        // The lifetime lock is per-workspace, so a live runtime is always this
        // workspace's own session.
        if (running) return verified;
