...
export function makeDevcontainerLifecycle(
  ...
  const composeContainerId = async (
    ...
    if ((await readJsonFile(registration.composePath)) === null) return null;
    const result = await docker(
      ["compose", "-f", registration.composePath, "ps", "-q", "agent"],
      { timeoutMs: 10_000 },
    );
    return Buffer.from(result.stdout).toString().trim() || null;
  };

  const composeDown = async (
    registration: DevcontainerRegistration,
  ): Promise<void> => {
    if ((await readJsonFile(registration.composePath)) === null) return;
  ...
  const verifyLive = async (
    ...
    const bytes = await readJsonFile(registration.configPath);
  ...
  const init = async (
    ...
    return await withDevcontainerOperationLock(host, workspace, async () => {
      ...
      await writeJsonFile(record.configPath, bytes);
      try {
        await writeJsonFile(
        ...
        // A config without its registration would make every later up fail the
        // fingerprint check with no way to re-init.
  ...
  const down = async (
    ...
    if (session)
      await withDevcontainerOperationLock(host, workspace, () =>
        writeDevcontainerSession(host, workspace, stopped),
      );
