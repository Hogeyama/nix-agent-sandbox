...
export async function readJsonFile(file: string): Promise<string | null> {
  try {
...
/** Atomic within the workspace operation lock; a partial file never becomes the record. */
...
export async function readDevcontainerRegistration(
  ...
  const bytes = await readJsonFile(paths.registrationFile);
...
export async function readDevcontainerSession(
  ...
  const bytes = await readJsonFile(
...
export async function writeDevcontainerSession(
  ...
  if (record.workspaceId !== devcontainerWorkspaceId(workspace))
    throw new DevcontainerError("session ownership mismatch");
  await writeJsonFile(paths.sessionFile, `${JSON.stringify(record)}\n`);
}
