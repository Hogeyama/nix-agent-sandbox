...
beforeEach(async () => {
  ...
  // Use a subdirectory so resolveSessionRuntimePaths can mkdir it.
  paths = await resolveSessionRuntimePaths(path.join(tmpRoot, "sessions"));
});
...
test("updateSessionTurn start transitions user-turn -> agent-turn and advances lastEventAt", async () => {
  ...
  expect(
    updated.lastEventAt >= initial.lastEventAt,
  ).toBe(true);
});
...
test("updateSessionTurn preserves startedAt across transitions", async () => {
  ...
  const afterAttention = await updateSessionTurn(
    paths,
    "sess-1",
    "attention",
  );
...
test("resolveSessionRuntimePaths honors NAS_SESSION_STORE_DIR env var", async () => {
  ...
  try {
    const resolved = await resolveSessionRuntimePaths();
    expect(resolved.sessionsDir).toBe(envSessionsDir);
    // Directory should have been created.
    const entries = await readdir(envSessionsDir);
