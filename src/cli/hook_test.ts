...
beforeEach(async () => {
  ...
  // that `resolveSessionRuntimePaths()` reads.
  process.env["NAS_SESSION_STORE_DIR"] = sessionsDir;
  paths = await resolveSessionRuntimePaths();
});
...
test("extractHookMessage: reads payload.notification.message", () => {
  expect(
    extractHookMessage({ notification: { message: "nested" } }),
  ).toBe("nested");
});

test("extractHookMessage: reads payload.Notification.message", () => {
  expect(
    extractHookMessage({ Notification: { message: "Capital" } }),
  ).toBe("Capital");
});
...
function makeSpyNotifier(): {
  calls: DesktopNotificationOptions[];
  notifier: (opts: DesktopNotificationOptions) => Promise<boolean>;
} {
  const calls: DesktopNotificationOptions[] = [];
  return {
    calls,
    notifier: async (opts: DesktopNotificationOptions) => {
      calls.push(opts);
      return true;
    },
  };
...
test("runHookNotification --kind attention with stdin message transitions to user-turn and records message", async () => {
  ...
  // Attention kind fires the notifier.
  expect(spy.calls.length).toBe(1);
  expect(spy.calls[0].title).toBe("nas: attention needed");
  expect(spy.calls[0].body).toBe("hello");
});

test("runHookNotification --kind attention without message uses fallback body", async () => {
  ...
  expect(spy.calls.length).toBe(1);
  expect(spy.calls[0].body).toBe("Agent needs your input");
});
...
test("runHookNotification --kind stop transitions to done", async () => {
  ...
  const spy = makeSpyNotifier();
  await runHookNotification(["--kind", "stop"], {
    stdinReader: emptyStdin,
    notifier: spy.notifier,
  });
  ...
  // stop does NOT fire the notifier.
...
test("runHookNotification tolerates notifier throwing and still exits cleanly", async () => {
  process.env["NAS_SESSION_ID"] = "sess-hook-throw";
  const { createSession, updateSessionTurn } = await import(
    "../sessions/store.ts"
  );
  await createSession(paths, {
    sessionId: "sess-hook-throw",
    agent: "claude",
    profile: "default",
    startedAt: "2026-04-11T10:00:00.000Z",
  });
  await updateSessionTurn(paths, "sess-hook-throw", "start");

  const throwingNotifier = async (): Promise<boolean> => {
    throw new Error("notify-send unavailable");
  };

  await runHookNotification(["--kind", "attention"], {
    stdinReader: makeStdin('{"message":"hi"}'),
    notifier: throwingNotifier,
  });

  const record = await readSession(paths, "sess-hook-throw");
  // Store update still happened even though notifier threw.
  expect(record?.turn).toBe("user-turn");
  expect(record?.lastEventMessage).toBe("hi");
});

test("runHookNotification: store update failure is swallowed, notifier not called", async () => {
  process.env["NAS_SESSION_ID"] = "sess-hook-store-fail";

  // Force `resolveSessionRuntimePaths` to throw by pointing
  // NAS_SESSION_STORE_DIR at a path whose parent is a regular file.
  // `ensureDir(runtimeDir)` will try to mkdir through a file component
  ...
  // --kind attention lets us assert the notifier is NOT called when the
  // store update has already failed before we reach the notification step.
  ...
  // Notifier must not be called: failure happened BEFORE we reach notify.
  expect(spy.calls.length).toBe(0);
