...
test(
  "local-proxy: forwards HTTP request with Proxy-Authorization",
  async () => {
...
test(
  "local-proxy: returns 502 when upstream is unreachable",
  async () => {
...
test(
  "local-proxy: CONNECT tunnel forwards with Proxy-Authorization",
  async () => {
...
test(
  "local-proxy: exits with error when NAS_UPSTREAM_PROXY is not set",
  async () => {
    const proc = Bun.spawn(["bun", "run", PROXY_SCRIPT], {
      stdout: "pipe",
      stderr: "pipe",
      env: { ...process.env, NAS_UPSTREAM_PROXY: undefined, NAS_LOCAL_PROXY_PORT: undefined },
    });
