...
class ServerUnpinTest(unittest.TestCase):
    def pinned(self):
        async def resolver(_host, port, **_kwargs):
            return [addrinfo("8.8.8.8", port)]

        addon = nas_addon.NasAddon(resolver=resolver, dns_timeout=0.1)
        data = FakeServerConnectData(("pinned.test", 443))
        asyncio.run(addon.server_connect(data))
        self.assertEqual(data.server.address, ("8.8.8.8", 443))
        return addon, data

    def test_connected_restores_host_name_so_the_connection_is_reused(self):
        addon, data = self.pinned()
        addon.server_connected(data)
        self.assertEqual(data.server.address, ("pinned.test", 443))
        self.assertEqual(addon._pinned_addresses, {})

    def test_connect_error_restores_host_name(self):
        addon, data = self.pinned()
        addon.server_connect_error(data)
        self.assertEqual(data.server.address, ("pinned.test", 443))
        self.assertEqual(addon._pinned_addresses, {})

    def test_unpinned_connection_is_left_alone(self):
        addon = nas_addon.NasAddon(dns_timeout=0.1)
        data = FakeServerConnectData(("8.8.8.8", 443))
        addon.server_connected(data)
        self.assertEqual(data.server.address, ("8.8.8.8", 443))
