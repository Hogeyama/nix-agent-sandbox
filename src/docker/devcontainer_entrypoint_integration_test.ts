...
test.skipIf(!imageBuildable)(
  `external linked worktree needs only workspace and canonical Git metadata mounts${!imageBuildable ? ` (${skipReason})` : ""}`,
  async () => {
    const root = await mkdtemp(path.join(tmpdir(), "nas-devcontainer-git-"));
    const repo = path.join(root, "main");
    const workspace = path.join(root, "external");
    const name = `nas-ide-git-${crypto.randomUUID()}`;
    try {
      for (const args of [
        ["init", repo],
        [
        ...
        ["-C", repo, "worktree", "add", "-b", "linked", workspace],
      ]) {
        const result = await run(["git", ...args]);
        expect(result.code, result.stderr).toBe(0);
      }
      await writeFile(path.join(repo, "host-only"), "not shared");
      const { resolveDevcontainerGitMetadata } = await import(
        "../stages/mount.ts"
      );
      const metadata = await resolveDevcontainerGitMetadata(workspace);
      expect(metadata).toEqual([path.join(repo, ".git")]);
      const mounted = await run([
        ...
        "-v",
        `${workspace}:${workspace}`,
        ...metadata.flatMap((source) => ["-v", `${source}:${source}`]),
        baseImage,
        "-c",
        'set -e; git -C "$1" status --porcelain; test ! -e "$2"',
        "nas-git",
        workspace,
        path.join(repo, "host-only"),
      ]);
      expect(mounted.code, mounted.stderr).toBe(0);
      expect(mounted.stdout).toBe("");
    } finally {
      await run(["docker", "rm", "-f", name]);
      await rm(root, { recursive: true, force: true });
    }
