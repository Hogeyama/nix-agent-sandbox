...
test("planDind: reserves remote container ports and the internal receiver", () => {
  const input = makeDindInput({ dockerEnable: true });
  input.profile.network.remoteForwards = [
    { hostPort: 8080, containerPort: 18081 },
  ];
  input.observability = { enabled: true, receiverPort: 4318 };
  expect(planDind(input)?.reservedPorts).toEqual([2375, 18080, 18081, 4318]);
});

test("planDind: absent forwarding reserves only actual processes", () => {
  expect(
    planDind(makeDindInput({ dockerEnable: true }))?.reservedPorts,
  ).toEqual([2375, 18080]);
});
...
test("DindStage: derives sidecar and volume names from the session", () => {
  ...
  expect(p.containerName).toEqual(
    "nas-dind-abcdef12-3456-7890-abcd-ef1234567890",
  );
...
test("reservedNamespacePorts: tolerates no forwarded ports", () => {
  expect(reservedNamespacePorts([])).toEqual([2375, 18080]);
});
...
test("DindStage: run calls ensureSidecar and teardownSidecar via DindService", async () => {
  ...
  expect(ensureCalls[0].containerName).toEqual("nas-dind-test-session-1234");
  ...
  expect(result.dind).toEqual({ containerName: "nas-dind-test-session-1234" });
  expect(result.container?.network).toEqual({
    mode: "container",
    containerName: "nas-dind-test-session-1234",
  });
