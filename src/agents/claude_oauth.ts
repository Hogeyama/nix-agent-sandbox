  ...
  constructor(detail: string) {
    super(
      `[nas] Claude OAuth credentials are not available on the host (${detail}). ` +
        `Run "claude /login" on the host, or set agentState.auth = "passthrough" (or new Mapping { ["claude"] = "passthrough" } for Claude only) to keep sharing the credentials file (required for API key use).`,
    );
